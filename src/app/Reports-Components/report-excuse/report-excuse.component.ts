...
export class ReportExcuseComponent implements OnInit {
  ...
  Filter() {
let exDate=this.datePipe.transform(this.NewExcuse.endDate, "dd-MM-yyyy")
console.log("dd",exDate)
console.log("dd2",this.NewExcuse.endDate)
    console.log("this.NewExcuse.startdate", this.NewExcuse.startdate);
    console.log("this.NewExcuse.endDate", this.NewExcuse.endDate);

    if (this.NewExcuse.professionID != 0) {
      this.Allexcuses = this.Allexcuses.filter(ex => ex.professionID == this.NewExcuse.professionID)
      this.FilteredExcuses = this.Allexcuses;

    }

    if (this.NewExcuse.employeeId != 0) {
      this.Allexcuses = this.Allexcuses.filter(ex => (ex.professionID == this.NewExcuse.professionID && ex.employeeId == this.NewExcuse.employeeId)
        || (ex.date >  this.NewExcuse.startdate && ex.date < this.NewExcuse.endDate))
      this.FilteredExcuses = this.Allexcuses;
      console.log("this.Allexcuses", this.Allexcuses);

    }

    // if (this.NewExcuse.startdate != exDate && this.NewExcuse.endDate != exDate) {

    //   let start=this.datePipe.transform(this.NewExcuse.startdate, "yyyy-MM-dd")
    //   let end=this.datePipe.transform(this.NewExcuse.endDate, "yyyy-MM-dd")

    //   this.Allexcuses = this.Allexcuses.filter(ex => ex.date >  start && ex.date < end)
    //   this.FilteredExcuses = this.Allexcuses;
    //   console.log("FilteredExcuses",this.FilteredExcuses)
    // }

    // var type2 = this.Allexcuses.filter(ex => 
    //    (ex.professionID == this.NewExcuse.professionID && ex.employeeId == this.NewExcuse.employeeId) 
    //    ||(ex.date>this.NewExcuse.startdate && ex.date<this.NewExcuse.endDate));
